...
export async function Hero() {
  ...
  const badge = block(blocks, "home.hero.badge", "Székesfehérvár és környéke");
  const headline = block(blocks, "home.hero.headline", "Minden cica megérdemel egy esélyt.");
  const subtext = block(
    blocks,
    "home.hero.subtext",
    "A Cat TNR Fehérvár gazdátlan, kóbor és rászoruló cicák mentésén, rehabilitációján és gazdásításán dolgozik."
  );
  const statNumber = block(blocks, "home.hero.stat_number", "80+ cica");
  const statCaption = block(blocks, "home.hero.stat_caption", "kapott már segítséget");
    ...
    <section className="relative overflow-hidden bg-cream-200 pb-16 pt-10 sm:pb-24 sm:pt-16">
      ...
      <div className="container-page relative grid items-center gap-12 lg:grid-cols-2 lg:gap-16">
        <div className="animate-fade-in-up">
          ...
          <p className="mt-6 max-w-lg text-lg leading-relaxed text-ink-500">{subtext}</p>
        ...
        <div className="animate-fade-in relative [animation-delay:150ms]">
          <div className="relative mx-auto max-w-md lg:max-w-none">
            ...
            <div className="absolute -bottom-6 -left-6 hidden rounded-xl2 bg-white p-4 shadow-lift sm:flex sm:items-center sm:gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-ink-900">{statNumber}</p>
                <p className="text-xs text-ink-400">{statCaption}</p>
              </div>
            </div>
          </div>
        </div>
